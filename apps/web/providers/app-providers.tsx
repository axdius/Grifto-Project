...
import {
  FakeCheckoutHost,
  InteractiveFakeCheckout,
} from "@/features/guest/services/interactive-checkout";
...
export function AppProviders({ children }: { children: ReactNode }) {
  ...
  const checkout = useMemo(() => new InteractiveFakeCheckout(), []);
    ...
    <QueryClientProvider client={queryClient}>
      <ApiClientProvider client={apiClient}>
        <PlatformServicesProvider services={services}>
          {children}
          <FakeCheckoutHost checkout={checkout} />
        </PlatformServicesProvider>
      </ApiClientProvider>
    </QueryClientProvider>
  );
